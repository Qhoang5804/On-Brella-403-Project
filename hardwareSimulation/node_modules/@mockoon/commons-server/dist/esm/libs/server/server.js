import { BINARY_BODY, BodyTypes, CORSHeaders, defaultEnvironmentVariablesPrefix, FileExtensionsWithTemplating, GetContentType, GetRouteResponseContentType, IsValidURL, MimeTypesWithTemplating, RouteType, ServerErrorCodes } from '@mockoon/commons';
...
import { ParsedJSONBodyMimeTypes, ParsedXMLBodyMimeTypes } from '../../constants/common.constants';
...
import { CreateCallbackInvocation, CreateTransaction, dedupSlashes, isBodySupportingMethod, preparePath, resolvePathFromEnvironment, routesFromFolder, stringIncludesArrayItems } from '../utils';
...
export class MockoonServer extends EventEmitter {
    constructor(environment, options = { envVarsPrefix: defaultEnvironmentVariablesPrefix }) {
        super();
            ...
            request.on('end', () => {
                ...
                try {
                    if (requestContentType) {
                        if (stringIncludesArrayItems(ParsedJSONBodyMimeTypes, requestContentType)) {
    ...
    createRequestListener() {
         ...
         */
        SetFakerLocale((_b = (_a = this.options.fakerOptions) === null || _a === void 0 ? void 0 : _a.locale) !== null && _b !== void 0 ? _b : 'en');
        SetFakerSeed((_d = (_c = this.options.fakerOptions) === null || _c === void 0 ? void 0 : _c.seed) !== null && _d !== void 0 ? _d : undefined);
        const app = express();
    ...
    setRoutes(server) {
        ...
        routes.forEach((declaredRoute) => {
            var _a;
            const routePath = preparePath(this.environment.endpointPrefix, declaredRoute.endpoint);
            if (!((_a = this.options.disabledRoutes) === null || _a === void 0 ? void 0 : _a.some((disabledRoute) => declaredRoute.uuid === disabledRoute ||
    ...
    createRouteHandler(route, requestNumber, crudId) {
        return (request, response, next) => {
            ...
            const enabledRouteResponse = new ResponseRulesInterpreter(currentRoute.responses, request, currentRoute.responseMode, this.environment, this.processedDatabuckets, this.globalVariables, this.options.envVarsPrefix).chooseResponse(requestNumber);
                    ...
                    if (enabledRouteResponse.bodyType === BodyTypes.DATABUCKET &&
                        ...
                        if (servedDatabucket) {
                            ...
                            if (route.type === RouteType.CRUD && crudId) {
                                content = databucketActions(crudId, servedDatabucket, request, response, currentRoute.responses[0].crudKey);
                            }
                ...
                try {
                    ...
                    setTimeout(() => {
                        fetch(url, {
                            ...
                            .catch((e) => this.emit('error', ServerErrorCodes.CALLBACK_ERROR, e, {
    ...
    serveBody(content, route, routeResponse, request, response, templateParse = true) {
        try {
            ...
            this.sendError(response, format(ServerMessages.ROUTE_SERVING_ERROR, error.message));
        }
    }
    ...
    sendFile(route, routeResponse, routeContentType, request, response) {
        const fileServingError = (error) => {
            ...
            this.sendError(response, format(ServerMessages.ROUTE_FILE_SERVING_ERROR, error.message));
        };
        ...
        if (this.environment.proxyMode &&
            ...
            server.use('*', createProxyMiddleware({
                ...
                onError: (error, request, response) => {
                    ...
                    this.sendError(response, `${format(ServerMessages.PROXY_ERROR, this.environment.proxyHost)} ${request.url}: ${error}`, 504);
                }
            }));
        }
    }
    ...
    emitCallbackInvoked(res, callback, url, requestBody, requestHeaders) {
        res.text().then((respText) => {
            ...
            this.emit('callback-invoked', CreateCallbackInvocation(callback, url, requestBody, reqHeaders, res, respText));
        });
    }
    /**
     ...
     */
    captureReferencedDatabucketIds(text) {
    ...
    findDatabucketIdsInCallbacks(response, environment) {
        if (response.callbacks && response.callbacks.length > 0) {
            for (const invocation of response.callbacks) {
                const callback = environment.callbacks.find((ref) => ref.uuid === invocation.uuid);
                if (!callback) {
                    continue;
                }
                const dataBucketIds = this.captureReferencedDatabucketIds(callback.body);
